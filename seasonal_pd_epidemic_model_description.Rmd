...
for(y in 1:numYears){
  ...
  ## (Re)set Sellke thresholds for recovery
  Rect <- rexp(numPlants, rate = 1)
  ...
  recoveryCheck <- cbind(Exp_times[expIndices], probRecovery[expIndices], infectionStatus[expIndices])
} # End y loop
...
#finalInfectionTimes <- floor(sapply(1:ncol(finalInfectionTimes), function(x) finalInfectionTimes[,x]+(52*(x-1))))
...
for(i in 1:numYears){
  ...
  infectionTimesList[[i]] <- data.frame(timeStep = infCounts$values+(52*(i-1)), 
                                        n = infCounts$lengths,
                                        cumuln = cumsum(infCounts$lengths))
}
...
ggplot(data = infectionTimes, aes(x = timeStep, y = cumuln)) +
  geom_path(lwd = 1) +
