...
EtDurationsPlot <- ggplot(data = TransitionData, aes(x = testTimes/4, y = Exposed_time)) +
  ...
  scale_x_continuous(name = "Month that plant became Exposed (infected)") +
   scale_y_continuous(name = "Time spent in Exposed compartment (weeks)", limits = c(0,35)) +
...
rho_et <- vectorDensity(A = -30, lambda_osc = 0.05, phi = 0.11, base = 0.2, time = timeVec)
...
qplot(x = timestep/30, y = test, geom = "path", xlab = "Week", ylab = "Probability of winter recovery")
