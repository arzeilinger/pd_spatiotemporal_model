...
## What's new 2019-12-11

\ 

Here's a new model update. I think I've covered all the pieces that we discussed last week.   
1. Time spent in the Exposed and Infectious compartments are dependent on the time of the year that each plant enters that compartment. The latent period (time spent in Exposed compartment) is considerably longer than time spent in the Infectious compartment.  
2. Overwintering vector infectivity (carryover) decays over the early season. Vector density does not carry over from previous year.  
3. Simulations are include vector overwintering and no roguing, no other scenarios are considered here.  
4. I plotted dynamics of E, I, and D compartments, total infectivity (in-field + immigrating), and beta:lambda ratio (i.e., beta/(beta + epsilon)).

\ 

Overall infection prevalence is much lower than the previous runs. I think this makes sense because the time plants spend in the Exposed compartment is much longer now, and baseline immigrating vector infectivity is lower. The dynamics of vector infectivity don't look great and don't vary much across the season, but the overall shape is about right.
...
Exposed_time <- transitionPeriod(a = 8, c = 0.02, t = testTimes) + Exposed_noise
...
parameterList <- list(
  ...
  kappa_e0 = 0.3, # Proportion of external vectors infectious
...
kappaPlot + scale_x_continuous(name = "Weeks", limits = endOfYears$timeStep[2:3])
