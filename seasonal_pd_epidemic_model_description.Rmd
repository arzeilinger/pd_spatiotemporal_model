  ...
  for(t in 1:Tmax){
    ...
    for(iiSusceptible in Susceptible_ind){
      ...
      if(vectorOverwintering & t == 1){
        kappa_e_initial <- kappaVec[Tmax] # External vector infectivity equals in-field infectivity at end of previous year
...
ggplot(data = plotkappa_t, aes(x = week, y = kappaVec)) +
  geom_path(lwd = 1) +
  scale_x_continuous(name = "Week", limits = c(1,52), breaks = seq(1,52,by=5)) +
