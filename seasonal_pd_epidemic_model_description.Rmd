...
muv_e <- 0.2 # Loss rate of vectors overwintering
...
for(y in 1:numYears){
  ...
  for(t in 1:Tmax){
    ...
    for(iiSusceptible in Susceptible_ind){
      ...
      #### ifelse() statement for vector overwintering
      ## If vectorOverwintering == TRUE, overwintering density is a fraction of the density in the field at the end of the previous year
      ## If vectorOverwintering == FALSE, overwintering density is 0
      overwinteringDensity <- ifelse(vectorOverwintering, (1-muv_e)*kappaVec[Tmax], 0)
      ## if() statement for case of first time step
      if(t == 1){
        rho_bt_tm1 <- 0
        rho_et_tm1 <- overwinteringDensity
      } else {
