...
Likewise, at the start of each growing season, the natural infectivity of immigrating vectors, $\kappa_{\epsilon}$, is set to the infectivity of in-field vectors at the end of the previous season, $\kappa_{\beta}(t = Tmax)$. We assume that infectious and non-infectious overwintering vectors die at the same rate.
...
kappaMatrix <- matrix(NA, nrow = Tmax, ncol = numYears)
...
for(y in 1:numYears){
  ...
  recoveryCheck <- cbind(Exp_times[expIndices, y], probRecovery[expIndices], recoveryStatus[expIndices])
  #### Removal and replanting of diseased hosts
  ...
  ## Assume that managers have are limited by the total number of plants they can remove and replant, rather than a fraction.
