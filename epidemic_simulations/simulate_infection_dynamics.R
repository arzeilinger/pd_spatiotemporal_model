...
tcuts <- floor(seq(1,Tmax, length.out = 10))
...
for(i in 1:length(tcuts)){
  ...
  raster.i <- setValues(raster.i, infectionStatus.i)
...
###########################################################################################
#### Looking at multiple simulation runs from cluster

spreadAdList <- readRDS("output/simulation_output_adrakey_setup_multiple_runs.rds")
str(spreadAdList)

## Looking at total infecteds
lapply(1:length(spreadAdList), function(x) sum(spreadAdList[[x]]$Inf_times < TmaxAd))



## Plotting one simulation run
simRun <- spreadAdList[[5]]
Inf_times <- simRun$Inf_times
## Inf_indices was not correctly calculated from simulateDiseaseSpread before 2019-10-23. Set up Inf_indices here
Inf_indices <- c(1:length(Inf_times))[order(Inf_times)]
...
hist(Inf_times[Inf_times < TmaxAd], breaks = seq(0,360,30))
