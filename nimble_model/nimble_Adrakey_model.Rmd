...
#### Set up simulated data set 
```{r}

#### This is the old toy data set
# Tmax <- 25
# alpha <- 3
# beta <- 1.0
# epsilon <- 0.05
...
sampler_infection <- nimbleFunction(
 ...
 methods = list(
   ...
   propose_add_infected = function(iPlant = double()) {
     ...
     if(!check_system()) {
       print(Inf_times_node)
       print(Inf_indices_node)
...
Configure MCMC
Also includes options to enter browser for debugging
...
## Run compiled MCMC
samples <- runMCMC(Cmcmc, niter = 1000, nchains=1)
...
samples_IT <- samplesDF %>% dplyr::select(contains("Inf_times"))
sdIT <- apply(samples_IT, 2, sd)
any(sdIT > 0)

plot(samples_IT[,365])

## Were the parameter changed by MCMC?
plot(samplesDF$alpha)
plot(samplesDF$beta)
plot(samplesDF$epsilon)

#### Check parameter estimates
median(samplesDF$alpha)
alpha

median(samplesDF$beta)
beta

median(samplesDF$epsilon)
epsilon
