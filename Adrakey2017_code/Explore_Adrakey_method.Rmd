...
dDiseaseSpread <- nimbleFunction(
  run = function(alpha = double(0), beta = double(0), epsilon = double(0), Coo = double(2), Inf_times = double(1), 
                 kernelnf = kernelnf, Tmax = double(0), numPlants = double(0), numInfections = double(0), 
...
dDiseaseObs <- nimbleFunction(
  run = function(x = double(0), Inf_indices = double(1), Inf_times = double(1), log = integer(0, default = 0)) {
    returnType(double(0))
...
code <- nimbleCode({
  ...
  ## To do: in the code above, Inf_indices only includes infected plants, i.e., length(Inf_indices) == numInfections....
  ## Need to decide if length(Inf_indices) should be numPlants instead
  Inf_indices[1:numPlants] <- dDiseaseSpread(alpha = alpha, beta = beta, epsilon = epsilon, 
                                                 Coo = Coo[1:numPlants, 1:2], Inf_times = Inf_times[1:numPlants], 
                                                 kernelnf = kernelnf, Tmax = Tmax, numPlants = numPlants, 
                                                 numInfections = numInfections, log = TRUE)
  Y[1:numPlants,1:2] <- dDiseaseObs(Inf_indices[1:numPlants], Inf_times[1:numPlants], log = TRUE)
})
...
data <- list(Inf_indices = Inf_indices,
             Y = Y)
