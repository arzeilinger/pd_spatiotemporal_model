...
sampler_infection <- nimbleFunction(
  ...
  run = function() {
    for(k in 1:trials) {
      ...
      if(time_intervals[iPlant, 2] > 2) {
        ## known symptomatic
        update_known_symptomatic(iPlant)
      } else {
        if(time_intervals[iPlant,1]==1 & time_intervals[iPlant, 2]==1) {
          ...
          ##  update_imputed_infection_time(iPlant)
          } else {
 ...
 methods = list(
   update_known_symptomatic = function(iPlant = double()) {
    currentLogProb <- model$getLogProb(calcNodes)
    currentPlaceInInfectionOrder <- 1
    while(model[[Inf_indices_node]][currentPlaceInInfectionOrder] != iPlant
          & currentPlaceInInfectionOrder <= numInfections) {
      currentPlaceInInfectionOrder <- currentPlaceInInfectionOrder + 1
    }
    if(currentPlaceInInfectionOrder > numInfections)
      print("Problem in update_known_symptomatic: currentPlaceInInfectionOrder > numInfections")
    
    current <- model[[Inf_times_node]][iPlant]
    proposal <- runif(1, time_intervals[iPlant, 1], time_intervals[iPlant, 2])

    if(proposal < current) {    
      proposedPlaceInInfectionOrder <- 1
      while(model[[Inf_times_node]][model[[Inf_indices_node]][proposedPlaceInInfectionOrder] ] < proposal &
            proposedPlaceInInfectionOrder <= currentPlaceInInfectionOrder) {
        proposedPlaceInInfectionOrder <- proposedPlaceInInfectionOrder + 1
      }
      if(proposedPlaceInInfectionOrder > currentPlaceInInfectionOrder)
        print("Problem in update_known_symptomatic: proposedPlaceInInfectionOrder > currentPlaceInInfectionOrder")
    } else {
      proposedPlaceInInfectionOrder <- currentPlaceInInfectionOrder
      while(model[[Inf_times_node]][model[[Inf_indices_node]][proposedPlaceInInfectionOrder + 1] ] < proposal &
            proposedPlaceInInfectionOrder <= numInfections) {
        proposedPlaceInInfectionOrder <- proposedPlaceInInfectionOrder + 1
      }
    }
    ...
    if(proposedPlaceInInfectionOrder != currentPlaceInInfectionOrder) {
      if(proposal > current) {
        moveStartFrom <-   currentPlaceInInfectionOrder + 1
        moveEndFrom <- proposedPlaceInInfectionOrder
        moveStartTo <- moveStartFrom - 1
        moveEndTo <- moveEndFrom - 1
      } else {
        moveStartFrom <- proposedPlaceInInfectionOrder
        moveEndFrom <- currentPlaceInInfectionOrder-1
        moveStartTo <- moveStartFrom + 1
        moveEndTo <- moveEndFrom + 1
      }
