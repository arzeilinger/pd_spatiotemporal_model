...
sampler_infection <- nimbleFunction(
  ...
  run = function() {
    for(k in 1:trials) {
      ...
      if(time_intervals[iPlant, 1] != model[[Tmax_node]]) {
        ...
        if(model[[Inf_times_nodes]][iPlant] < model[[Tmax_node]]) {
 ...
 methods = list(
   ...
   propose_add_infected = function(iPlant = double()) {
     ...
     if(!check_system()) {
       print("Found the system out of valid state entering propose_add_infected")
       browser()
     }
     current_numInfections <- model[[numInfections_node]]
     if(current_numInfections < numPlants) {
     ...
     if(!check_system()) {
       print("Found the system out of valid state exiting propose_add_infected")
       browser()
     }
   ...
   propose_remove_infected = function(iPlant = double) {
     ...
     if(!check_system()) {
       print("Found the system out of valid state entering propose_remove_infected")
       browser()
     }
     ...
     if(current_numInfections > 0) {
       ...
       if(!check_system()) {
         print("Found the system out of valid state exiting propose_remove_infected")
         browser()
       }
   ...
   update_infection_time = function(iPlant = double(),
     ...
     if(!check_system()) {
       print("Found the system out of valid state entering update_infection_time")
       browser()
     }
    ...
    if(!check_system()) {
      print("Found the system out of valid state in update_infection_time before calculating proposal logProb.")
      browser()
    }
    ...
    if(!check_system()) {
      print("Found the system out of valid state exiting update_infection_time.")
      browser()
    }
...
```{r}
## Empty configuration
MCMCconf <- configureMCMC(Rmodel, nodes = NULL) #c("alpha", "beta", "epsilon"))
...
MCMCconf$addMonitors('Inf_times', 'Inf_indices')
...
#### To run many MCMC iterations for debugging purposes
set.seed(1)

samples <- MCMC$run(niter = 100)

## Not sure why but "samples" is empty when I use MCMC$run, but returns a matrix when I use runMCMC(MCMC)
samples <- runMCMC(MCMC, niter = 100000, nchains=1)
saveRDS(samples, file = "MCMC_sample_infection_raw_output.rds")

## Were Inf_indices changed by MCMC?
library(dplyr)
samples_II <- samples %>% as.data.frame() %>% dplyr::select(contains("Inf_indices"))
summary(samples_II)
