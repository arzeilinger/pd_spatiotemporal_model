...
parameterList <- list(
  ...
  #### Winter recovery
  b_recovery = -0.045, # Shifting the recovery curve
  #### Defining seasonal changes in duration of Exposed compartment (latent period)
  a_transition = 8,
  c_transition = 0.004
)
...
parameterList3 <- list(
  ...
  #### Winter recovery
  b_recovery = parameterList$b_recovery, # Shifting the recovery curve
  a_transition = parameterList$a_transition,
  c_transition = parameterList$c_transition
)
...
for(i in 1:nruns){
  print(paste("On simulation run ", i, sep = ""))
  sim4List[[i]] <- seasonalPDmodel(parameterList4,
                                             nrc = nrc,
                                             Tmax = Tmax,
                                             numYears = numYears,
                                             numPlantsRogued = 0, # No roguing
...
for(i in 1:length(kappa_b_list)){
  ...
  if(i <= 4){
    ...
    kappaDetailPlots[[i]] <- kappaPlot + 
      scale_x_continuous(name = "Weeks", limits = endOfYears$timeStep[8:9]) +
      scale_y_continuous(name = "Proportion of vectors infectious", limits = c(0,1))
  } else {
    kappaDynamicsPlots[[i]] <- kappaPlot + 
      scale_x_continuous(name = "Weeks") +
